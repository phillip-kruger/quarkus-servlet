...
public class HttpSessionContext implements InjectableContext, HttpSessionListener {
    ...
    private HttpSession session(boolean create) {
        ...
        try {
            ServletRequestContext ctx = ServletRequestContext.get();
            if (ctx != null) {
    ...
    public void sessionCreated(HttpSessionEvent se) {
        Arc.container().beanManager().getEvent()
                .select(HttpSession.class, Initialized.Literal.SESSION)
                .fire(se.getSession());
    }
    ...
    static class Key {
        ...
        Key(CreationalContext<?> creationalContext, String beanIdentifier) {
